    ...
    <div style={{ background: '#161a20', border: '1px solid #2a2f38' }} className="rounded-xl p-6">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <div className="rounded-lg p-3" style={{ background: '#0d0f12', border: '1px solid #2a2f38' }}>
          <p className="text-xs" style={{ color: '#6b7280' }}>Active partners</p>
          <p className="text-lg font-mono font-bold text-white">{inputs.numberOfPartners}</p>
          <p className="text-xs" style={{ color: '#4b5563' }}>{inputs.partnerTestsPerDayEach} tests/day each</p>
        </div>
        <div className="rounded-lg p-3" style={{ background: '#0d0f12', border: '1px solid #2a2f38' }}>
                ...
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-mono" style={{ color: '#6b7280' }}>
                      {row.dailyDemand.toFixed(1)} tests/day needed
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
      ...
      {(() => {
        ...
        if (!firstOverYear) return (
          <p className="text-xs mt-4" style={{ color: '#6b7280' }}>
            Current {inputs.numberOfPartners} partners are sufficient for the full forecast period.
          </p>
        )
        return (
          <div className="rounded-lg px-4 py-3 mt-4"
            ...
            <p className="text-xs" style={{ color: '#9ca3af' }}>
              ...
              <span className="font-mono font-bold text-white">{firstOverYear.partnersNeeded}</span>
              {' '}partners by then — sign{' '}
              <span className="font-mono font-bold text-white">
                {Math.max(0, firstOverYear.partnersNeeded - inputs.numberOfPartners)}
              </span>
              {' '}more.
            </p>
          </div>
